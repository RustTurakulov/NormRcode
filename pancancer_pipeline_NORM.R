...
if (parameter[1] == "NOP") {
    anno_new      <- read.table(newsamplesheet, header=F, sep="\t", quote="")  ## getting just one column of barcodes for new samples
	allnewsamples <- as.character(unlist(anno_new[,1]))
	anno_base <- anno_base[anno_base$idat_filename %in% allnewsamples, ] 
	anno <- anno_base
}else if(parameter[1] == "Sarcoma"){
...
if(is.na(newsamplesheet)){
    ...
    if(parameter[1] != "NOP"){
		...
		newdirs = unique(matrix(unlist(strsplit(anno_new[,1], "_")), ncol=2, byrow=TRUE)[,1])
	...
	#			 rawsheet <- rawsheetfull[grepl("Clinical|Brain|Sarcoma|CBTN", sampletype, ignore.case = TRUE, perl = TRUE), ]; ## Only some rows/types go in
				 ...
				 NCI_METRIC           = rawsheet[,which( names(rawsheet) == 'Class1')];
		...
		newsamplesheet <- newsamplesheet[newsamplesheet$idat_filename %in% anno_new[,1],]
		...
		"~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
		write.csv(newsamplesheet, file.path(batchdirout,"newsamples.csv"), row.names=F);
	...
	##### Blending new samples with Drew's database {anno_base}
	##### Column names may change without notice!
		...
		anno_base_new$RFpurity.ESTIMATE <- newsamplesheet$RFpurity.ESTIMATE
		anno_base_new$RFpurity.ABSOLUTE <- newsamplesheet$RFpurity.ABSOLUTE
		...
		anno_base_new$material_prediction <- newsamplesheet$Material_Type
		...
		anno_base_new$CNS.MCF          <- newsamplesheet$CNS.MCF
		anno_base_new$CNS.MCF.score    <- newsamplesheet$CNS.MCF.score 
...
#identify bystander samples
    ...
    if(parameter[1] == "NOP"){
	    newsmplsfailedqc <- bad_samples_all; # No Cleanup for NOP case
    }else{
...
##### Still bloody slow (> 1day on CNS set)
prcomp_svds <-

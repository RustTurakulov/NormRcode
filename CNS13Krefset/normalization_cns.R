...
## 11/10/2021 :: With two UMAP plots on output 
...
## Putting both samplesheets together without the last Basename column 
combo_samplesheet <- rbind(newsamplesheet[,-33], anno_base);

fwrite(combo_samplesheet, file.path(batchdirout, "combo_samplesheet.tsv"),
       sep = "\t", row.names = TRUE,  nThread = 12);

## Reshaping of truncated rawsamplesheet with new samples annotation for meffil format
...
meffil.qc.report(new.qc.summary, output.file=file.path(batchdirout,"qc_report.html"))

# Generate  major reference set 13K samples
# Has to be done once a months when toped up with new samples other times just comment 
...
message("\n\nDone quantiles normalization:   ", Sys.time(), "\n now extracting new batch beta...");
options(mc.cores=36)  ## 36 for large memory 
cores <- options()$mc.cores
gc();
...
message("\n\nDone quantiles normalization for new set:   ", Sys.time(), "\n now for reference set...");
...
rm(list=setdiff(ls(), c("new.norm.beta", "ref.norm.betas", "combo_samplesheet", "batchdir", "batchdirout", "cores", "bad.cpgs")));
gc()
...
bad.cpgs <- readRDS(file.path(batchdirout, "bad.cpgs.longlist.rds"))
message("\nRemoving ", length(bad.cpgs), " of bad cpgs: mt, X, Y, SNPs\n");
beta_filtered <- beta[!rownames(beta) %in% bad.cpgs,]
rm(beta)
gc();

message("\nReduce beta values by variable probes with SD >", 0.27);
beta_filtered <- t(beta_filtered)
gc();
#saveRDS(beta_filtered, file.path(batchdirout, "beta_filtered_transposedfull.rds")); #slow
#beta_reduced  <- beta_filtered[,order(-apply(beta_filtered,2,sd))[1:10000]]
beta_reduced <- beta_filtered[,Rfast::colVars(as.matrix(beta_filtered), std = TRUE, parallel = TRUE)>0.27]; 
write.table(beta_reduced, file.path(batchdirout, "combo_beta_topSDprobes_pc14.tsv"),  sep = "\t", row.names = TRUE);
beta_reduced  <- fread(file.path(batchdirout,"combo_beta_topSDprobes_pc14.tsv"), sep = "\t", nThread = cores);
